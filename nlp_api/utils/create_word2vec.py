...
import os
import math
from . import fileprocessing,segment
...
def save_multi_file(files_list,labels_list,word2vec_path,out_dir,prefix,batchSize,max_sentence_length,
    ...
    # 把该目录下的所有文件都删除
    fileprocessing.delete_dir_file(out_dir)
    ...
    if labels_set is None:
        labels_set= fileprocessing.get_labels_set(labels_list)

    labels_list, labels_set = fileprocessing.labels_encoding(labels_list, labels_set)
    ...
    for i in range(batchNum):
        ...
        # 读取文件内容，字词分割
        batch_content = fileprocessing.read_files_list_to_segment(batch_files,
                                                                    max_sentence_length,
                                                                    padding_token='<PAD>',
                                                                    segment_type='word')
        # 将字词转为索引矩阵
...
def pos_neg_test():
    ...
    sentences, labels = fileprocessing.load_pos_neg_files(positive_data_file, negative_data_file)
...
if __name__ == '__main__':
    ...
    # 读取所有文件列表
    files_list, label_list = fileprocessing.gen_files_labels(THUCNews_path)
    ...
    # 获得标签集合，并保存在本地
    # labels_set=['星座','财经','教育']
    # labels_set = fileprocessing.get_labels_set(label_list)
    labels_file='../data/THUCNews_labels.txt'
    # fileprocessing.write_txt(labels_file, labels_set)

    # 将数据划分为train val数据集
    train_files, train_label, val_files, val_label= fileprocessing.split_train_val_list(files_list, label_list, facror=0.9, shuffle=True)

    # contents, labels=fileprocessing.read_files_labels(files_list,label_list)
    # word2vec_path = 'out/trained_word2vec.model'
    train_out_dir='../data/train_data'
    prefix='train_data'
    batchSize=20000
    labels_set=fileprocessing.read_txt(labels_file)
    # labels_set2 = fileprocessing.read_txt(labels_file)
